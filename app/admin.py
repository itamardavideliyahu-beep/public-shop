...
class ListingAdminView(SecureModelView):
    ...
    column_list = ['id', 'title', 'price', 'category', 'status', 'seller', 'created_at', 'expires_at']
    column_searchable_list = ['title', 'description']
    column_filters = ['category', 'status', 'created_at']
    form_columns = ['title', 'description', 'price', 'category', 'status', 'seller', 'expires_at']
...
def setup_admin(app):
    ...
    admin.add_view(ListingAdminView(Listing, db.session, name='Listings', category='Models'))
    admin.add_view(PostAdminView(Post, db.session, name='Posts', category='Models'))
    admin.add_view(ConversationAdminView(Conversation, db.session, name='Conversations', category='Models'))
    admin.add_view(MessageAdminView(Message, db.session, name='Messages', category='Models'))
