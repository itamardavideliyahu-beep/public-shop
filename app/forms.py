...
class PostForm(FlaskForm):
    ...
    content = TextAreaField('Content', validators=[DataRequired(), Length(min=3, max=5000)])
...
class ListingForm(FlaskForm):
    ...
    price = FloatField('Price', validators=[DataRequired(), NumberRange(min=0.01, max=999999999)])
    category = SelectField('Category', validators=[DataRequired()])
    image = FileField('Image', validators=[Optional(), FileAllowed(['jpg', 'jpeg', 'png', 'gif', 'webp'], 'Images only!')])
    expires_days = FloatField('Expiration Days', validators=[Optional(), NumberRange(min=1, max=365)], default=365)
