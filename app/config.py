...
class Config:
    ...
    MAIL_DEFAULT_SENDER = os.environ.get("MAIL_DEFAULT_SENDER", "noreply@publicshop.com")
...
class ProductionConfig(Config):
    ...
    def init_app(cls, app):
        ...
        if not secret_key or secret_key == "dev-secret-change-this-in-production":
            raise ValueError("SECRET_KEY must be set in production environment! Generate with: python -c 'import secrets; print(secrets.token_hex(32))'")
        
        # Validate email configuration
        if not app.config.get("MAIL_USERNAME") or not app.config.get("MAIL_PASSWORD"):
            app.logger.warning("Email configuration not set. Email verification will not work!")
